...
它不是完整前端项目，不是完整社区平台，也不是普通聊天机器人。它是面向校园交流社区与求助任务平台的 Agent 后端服务层。
...
严禁把队友的完整社区数据库和完整社区业务复制进本项目。本项目只能通过 adapter 调用队友社区接口，可保存外部 ID，例如 `external_user_id`、`source_post_id`、`created_task_id`。
...
前期对接 mock community API，后期替换为队友真实社区接口。
