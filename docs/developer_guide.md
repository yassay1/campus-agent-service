...
| `app/services/community_client.py` | 队友 API | 对接真实社区 |
...
1. 队友社区服务启动后，配置 `.env` 中的 `COMMUNITY_SERVICE_BASE_URL`
2. Agent Service 调用队友 API 的记录会存在 `external_api_calls` 表
3. 队友需要从 Agent Service 获取的数据通过 `POST` 接口传参，不走数据库直连
4. ID 关联约定见 `docs/teammate_integration.md`
...
**Q: task_drafts 和队友的 help_tasks 什么关系？**
`task_drafts` 是 Agent 生成的草稿（存在我的数据库）。队友确认后通过 CommunityClient 创建正式 `help_tasks`（存在队友数据库）。草稿的 `created_task_id` 存队友返回的正式任务 ID。
...
1. **约定 API 格式**：与队友对齐 `docs/teammate_integration.md` 中的接口
2. **配置 `COMMUNITY_SERVICE_BASE_URL`**
3. **联调 `CommunityClient`**：在队友 API 可用后测试 `create_task()`、`get_user_info()` 等方法
4. **查看 `external_api_calls` 表**：每次调用队友 API 都会记录，方便排查
